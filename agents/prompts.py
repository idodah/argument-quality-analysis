...
INITIAL_GEN_USER = (
    ...
    "### Post\n{post}\n\n"
    "{prior_attempt_note}"
    "Write TWO distinct responses to this post, each taking a different angle. "
    "Each must be SHORT: one or two paragraphs, no more. Identify the specific "
    "trope the post advances and refute it with the documented record. You will "
    "not fit the myth's full history — pick the ONE or TWO most decisive facts "
    "(the forgery that was proven, the study that found no evidence, the record "
    "that contradicts the claim) and lead with them. Correct the claim; do not "
    "characterize the poster. Concision is a requirement, not a preference: a "
    "tight two-paragraph reply gets read, a six-paragraph one does not.\n\n"
    "Write it as plain prose, the way a person types a reply: no bold, no "
    "headings, no bullet lists. Heavy **emphasis** reads as machine-generated "
    "and costs you the reader.\n\n"
...
REFLECT_SYSTEM = (
    ...
    "Give a short critique (3-6 bullets) under two headings. Be concrete — point "
    "to specific claims. Do NOT rewrite the comment.\n\n"
    "LENGTH BUDGET: the final reply must be ONE OR TWO PARAGRAPHS. It cannot "
    "absorb everything, so critique it as an editor working to that cap: name "
    "only the highest-value change per bullet, and prefer a swap ('replace X "
    "with the stronger Y') over a pure addition. Do not ask for more material "
    "than two paragraphs can hold.\n\n"
    "**Missing** (what to ADD): the most important point the comment fails to "
    "address, and the factual gap where the retrieved evidence would most "
    "strengthen it (reference docs by [url]/[title] if helpful). Frame gaps as "
    "documented refutations of the trope. At most 3 bullets here — if the reply "
    "is already at the cap, say what to cut to make room.\n"
...
REFINE_SYSTEM = (
    ...
    "HARD LIMIT: one or two paragraphs. This cap outranks the critique — CUT the "
    "'**Superfluous**' items first, then add only the '**Missing**' items that "
    "still fit, strongest first. Dropping a valid suggestion to stay within two "
    "paragraphs is correct behaviour, not a failure; do NOT pad, and do not add "
    "a third paragraph to fit everything. Fix notes and groundedness issues are "
    "the exception: resolve every item in '### Fix Notes', trimming elsewhere to "
    "make room. Output ONLY the revised argument, addressed to the poster as "
    "'you', in plain prose — no bold, headings, or bullet lists (heavy "
    "**emphasis** reads as machine-generated). Don't reintroduce problems fixed "
    "in '### Critique History' (oldest first).\n\n"
