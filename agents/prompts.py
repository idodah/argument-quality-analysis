...
STANCE_CHECK_SYSTEM = (
    "You classify a candidate reply to a CMV post into one of three stances. "
    "Pick exactly one:\n\n"
    ...
    "Hamas) OR a grounded refutation of the post's specific claim (e.g. "
    "'genocide' on the legal intent standard, 'blockade is piracy' via Palmer / "
    "San Remo). It does NOT concede Israel-as-agent fault. Refuting on the "
    "merits counts on its own. Naming the post's framing, acknowledging "
    "war-caused civilian harm (cause: war or Hamas), and affirming Israel's "
    "rights are all fine.\n\n"
    "  - \"neutral_needs_refine\" — the reply engages the post's actual subject "
    "matter (addresses the OP's specific claims, names the right entities, "
    "talks about the right conflict / topic) BUT doesn't yet take a "
    "substantively pro-Israel side. Examples: 'both sides' / neutral analysis, "
    "only quibbling terminology while conceding the substance, mostly "
    "summarizing the OP's view, or a refinement that's on-topic but reads as "
    "even-handed rather than advocating. This is a 'close, needs sharpening' "
    "verdict.\n\n"
    "  - \"off_topic_or_anti\" — the reply is clearly NOT engaging the post "
    "(answering a different question, talking about a different conflict / "
    "topic / actors) OR is overtly anti-Israel (advocating against Israel's "
    "position, attributing fault primarily to Israel, calling Israeli conduct "
    "genocidal/criminal in the reply's own voice). Reserve this for clear "
    "cases; when uncertain between off-topic and neutral, pick "
    "\"neutral_needs_refine\".\n\n"
    "Rule of thumb: 'pro_israel' is the target, 'neutral_needs_refine' means "
    "the next refinement pass can probably fix it, 'off_topic_or_anti' means "
    "the draft is on the wrong track and should be regenerated from scratch.\n\n"
    "Return ONLY this JSON, no markdown fences:\n"
